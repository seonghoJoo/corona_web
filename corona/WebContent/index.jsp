...
<html lang="ko">
	...
	<body>
		<%@ include file="/WEB-INF/template/header.jsp" %>
			...
			<%if(trips.size() == 0){ %>
				...
				<li>
					<h3>
						...
						<a href="/delete.do?no=<%=trip.getNo() %>" class="delete btn" data-gu="<%=trip.getGu() %>" data-num="<%=trip.getNum()%>"><i class="far fa-trash-alt"></i> 삭제</a>
			...
			<%} }%>
				...
				<div class="btn_box">
					...
					<a class="btn" href="/logout.bmj"><i class="fas fa-door-open"></i> 로그아웃</a>
				</div>
				<%= paginate%>
		...
		$('.delete').on("click",function () {
		    ...
		    const no = $(this).attr("data-num");
		    const gu = $(this).attr("data-gu");
		    //const name = $(this).data('num');
			 if (confirm(gu+" "+ no+"번째 확진자를 정말로 삭제하시겠습니까??") == true){    //확인
